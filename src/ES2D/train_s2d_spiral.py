...
def loss_naima_weighted(disp_pred, disp, face_pred, face, loss_weights):
    ...
    for epoch in range(starting_epoch, args.epochs + 1):
        ...
        for b, sample in pbar_train:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = loss_naima_weighted(disp_pred, vertices-template, vertices_pred, vertices, loss_weights) 
        ...
        if epoch % 20 == 0 or epoch == args.epochs:
            ...
            with torch.no_grad():
                ...
                for b, sample in pbar_val:
                    ...
                    disp_pred, vertices_pred = s2d.forward(vertices_land - template_land, template)
                    loss = criterion_test(vertices_pred, vertices) 
                    vloss += loss.item()

                    pbar_val.set_description(
                        "(Epoch {}) TEST MVE Talking:{:.10f}".format((epoch + 1), vloss/(b+1)))
                
               
        torch.save({'epoch': epoch,
            'autoencoder_state_dict': s2d.state_dict(),
            'optimizer_state_dict': optim.state_dict(),
            }, os.path.join(args.result_dir, 's2d_Spiral_Naima_Loss_New_Training_More_Emotions.pth.tar'))
...
def main():
    ...
    parser.add_argument("--result_dir", type=str, default="/mnt/diskone-first/TH/New_S2D/saves")
    ...
    parser.add_argument("--root_dir", type=str, default='/mnt/diskone-second/emotional_vocaset_smaller/training_data')
